...
public:
	...
	virtual sp<IStreamOut> gethandle()
	{
		Parcel data, reply;
		sp<IStreamOut> audiosynergy;
		data.writeInterfaceToken(IAudioFlinger::getInterfaceDescriptor());
	
		status_t lStatus = remote()->transact(GET_HANDLE, data, &reply);
		if (lStatus != NO_ERROR) {
			ALOGE("gethandle error: %s", strerror(-lStatus));
		} else {
			audiosynergy = interface_cast<IStreamOut>(reply.readStrongBinder());
		}
		return audiosynergy;
	}
	...
	virtual void closehandle()
	{
		Parcel data, reply;
		data.writeInterfaceToken(IAudioFlinger::getInterfaceDescriptor());
		status_t lStatus = remote()->transact(CLOSE_HANDLE, data, &reply);
	}

	virtual void time_delay_slave(long time_java)
	{
		ALOGE("vaylb_test-->IAudioFlinger: time_delay_slave run");
		Parcel data, reply;
		data.writeInterfaceToken(IAudioFlinger::getInterfaceDescriptor());
		data.writeInt32(time_java);
		remote()->transact(TIME_DELAY_SLAVE, data, &reply);
	}
};
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    switch (code) {
		...
		case GET_HANDLE: {
            ALOGE("audio_test-->IAudioFlinger::onTransact: gethandle() run, path:IAudioFlinger.cpp");
            CHECK_INTERFACE(IAudioFlinger, data, reply);
            
            sp<IStreamOut> audiosynergy;
			audiosynergy = gethandle();
            reply->writeStrongBinder(audiosynergy->asBinder());
            return NO_ERROR;
        } break;
		case CLOSE_HANDLE: {
            ALOGE("audio_test-->IAudioFlinger::onTransact: closehandle() run, path:IAudioFlinger.cpp");
            CHECK_INTERFACE(IAudioFlinger, data, reply);
			closehandle();
            return NO_ERROR;
        } break;
		case TIME_DELAY_SLAVE: {
            CHECK_INTERFACE(IAudioFlinger, data, reply);
            time_delay_slave(data.readInt32());
            return NO_ERROR;
        } break;
        default:
