...
sp<IStreamOut> AudioFlinger::gethandle()
{
    	audio_io_handle_t output = get_current_output();
	return getThreadStreamOutHandle(output);
}


sp<AudioFlinger::StreamOutHandle> AudioFlinger::getThreadStreamOutHandle(audio_io_handle_t handle)
{
	sp<PlaybackThread> thread = checkPlaybackThread_l(handle);
	sp<StreamOutHandle> streamouthandle;
	ALOGE("vaylb_test-->Thread->mStreamOutHandle is NULL ,new one and sort it.");
	audio_stream_out_t* streamout = thread->mOutput->stream;
	streamouthandle = new StreamOutHandle(thread,streamout);
	thread->mStreamOutHandle = streamouthandle;
    	return streamouthandle;
}

void AudioFlinger::closehandle()
{
	ALOGE("vaylb_test-->AudioFlinger::closehandle(), path:AudioFlinger.cpp");
	
   // audio_io_handle_t output = AudioSystem::getOutput(AUDIO_STREAM_MUSIC,441000, AUDIO_FORMAT_PCM_16_BIT, AUDIO_CHANNEL_OUT_STEREO,AUDIO_OUTPUT_FLAG_NONE,NULL);
	//audio_io_handle_t output = AudioSystem::getCurrentOutput(); 
	sp<PlaybackThread> thread = checkPlaybackThread_l(vaylb_current_otput);
	if(thread->mStreamOutHandle != NULL)
	{
		thread->mStreamOutHandle.clear();
		// thread->mStreamOutHandle = NULL;
	}
    return;
}
audio_io_handle_t AudioFlinger::get_current_output()
{
	audio_io_handle_t output = 2;
	sp<PlaybackThread> thread = checkPlaybackThread_l(2);
	if(thread->mActiveTracks.size() > 0)  output = 2;
	//thread = checkPlaybackThread_l(3);
	//if(thread->mActiveTracks.size() > 0)  output = 3;
	ALOGE("vaylb_test-->AudioFlinger:get_current_output: output = %d",output);
	vaylb_current_otput = output;
	return output;
}

void AudioFlinger::time_delay_slave(long time_java){
	ALOGE("vaylb_test-->AudioFlinger: time_delay_slave run.");
	audio_io_handle_t output = AudioSystem::getOutput(AUDIO_STREAM_MUSIC,441000, AUDIO_FORMAT_PCM_16_BIT, AUDIO_CHANNEL_OUT_STEREO,AUDIO_OUTPUT_FLAG_NONE,NULL);
	if (output == 0) {
    	ALOGE("Could not get audio output for stream type %d", AUDIO_STREAM_MUSIC);
		return;
    }
	sp<PlaybackThread> thread = checkPlaybackThread_l(output);
	stream_out* streamout = (stream_out*)thread->mOutput->stream;
	streamout->time_delay_slave = time_java;
	streamout->time_delay_slave_flag = true;
	return;
}
